...
public class AreaFill {
   ...
   public static char[][] read(String filename) {
      ...
      String[] size = infile.nextLine().split(" ");
      char[][] arr = new char[Integer.parseInt(size[0])][Integer.parseInt(size[1])];
      for (int row = 0; row < Integer.parseInt(size[0]); row++) {
         String s = infile.nextLine();
         for (int col = 0; col < Integer.parseInt(size[1]); col++) {
   ...
   public static void fill(char[][] g, int r, int c, char ch) {
      ...
      if (r > 0 && g[r - 1][c] == ch) { // up
         fill(g, r - 1, c, ch);
      }
      if (r < g.length - 1 && g[r + 1][c] == ch) { // down
         fill(g, r + 1, c, ch);
      }
      if (c > 0 && g[r][c - 1] == ch) { // left
         fill(g, r, c - 1, ch);
      }
      if (c < g[0].length - 1 && g[r][c + 1] == ch) { // right
         fill(g, r, c + 1, ch);
      }
   }
