...
public class Infix_Extension
{
   ...
   {
      if (!ParenMatch.checkParen(infix)) return "ERROR in parentheses";
      ...
      for (int i=0; i<nums.size(); i++) {
         String s = nums.get(i);
         if ((s.charAt(0)=='-' && s.length()>1) || Character.isDigit(s.charAt(0))) post+=s+" ";
         else if (LEFT.indexOf(s)>=0) st.push(s);
         else if (RIGHT.indexOf(s)>=0) {
            while (!st.isEmpty() && LEFT.indexOf(st.peek())<0) post+=st.pop()+" ";
            st.pop();
         }
         else {
            if (getLevel(s)<0) return "ERROR non-algebraic symbol";
