...
def taxiMine(board):
    d = 0
    for ind, val in enumerate([ord(i)-64 if i != "." else i for i in board]):
        if val != ".":
...
def taxicab(n):
    ...
    for i, val in enumerate(n):
        if val != ".":
            desiredInd = alphaDict[alpha[alpha.index(
                val)]] if alphaCheck != -1 else int(val) - 1
...
def specRowCol(cPerm, desired):  # nth row/col, "R" for row, "C" for col, permutation, desired
    conflict = 0
    # desiredRow = desired[nth * 4: nth * 4 + 1]
    current = ""
    for i in cPerm:
...
def precalculate(size):
    all = permutations(find_5x5(), size)  # permutations(find_4x4(), size)#
    # size = 4
    desired = find_5x5()  # find_goal_korf()#find_5x5()#
    for z in all:
        for i in range(0, size):
            saveDict[(i, "R", "".join(z))] = specRowCol(
                z, desired[i * size: i * size + size])
        colIndex = [0, 5, 10, 15, 20]  # [0, 4, 8, 12]
        for i in range(0, size):
            desiredCol = ""
            for q in colIndex:
...
def total_board_conflicts(board):
    ...
    for i in range(N):
        ...
        conflicts += c_dct[("".join([board[c+i] for c in col]), i)]
    return conflicts
        ...
        if v not in closed:
            ...
            for c in get_children(size, v):
                if c not in closed:
                    ...
                    col = [0, 4, 8, 12]
                    for i in range(0, size):
                        conflicts += saveDict[(i, "R",
                                               c[i * size:i * size + size])]
                    for i in range(0, size):
                        colC = ""
                        for u in col:
                            colC += c[u + i]
...
# generate_dcts("ABCDEFGHIJKLMNOPQRSTUVWX.")

# print('done creating dicts')
...
for i in lines:
    ...
    p = paritykorf(i, size)
    if p == True:
       print("Line " + str(count) + ": " + i + ", A* " + str(a_star(i)) +
             " moves in " + str(perf_counter() - s) + " seconds")
    else:
      print("Line " + str(count) + ": " + i +
            ", no solution, found in " + str(perf_counter() - s) + " seconds")
    # if p == True:
