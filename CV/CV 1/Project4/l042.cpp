...
bool part1comp(const Point& A, const Point& B) {
...
void part1() {
    ...
    while (pin >> x >> y)
        p.emplace_back(x, y);
    Point& minp = *min_element(p.begin(), p.end(), part1comp);
    Point& maxp = *max_element(p.begin(), p.end(), part1comp);
...
bool part2comp_min(Point& A, Point& B) {
    return (A.getY() < B.getY()) || (A.getY()==B.getY() && A.getX()<B.getX());
}

bool part2comp_sort(Point& A, Point& B) {
    double first = (A.getX()-px)/dist(P, A), second = (B.getX()-px)/dist(P, B);
    if (abs(first-second)<epsilon)
        return A.getX()>B.getX();
    return first > second; // lower cos = higher angle
}
...
void part2() {
    ...
    while (pin >> x >> y)
        p.emplace_back(x, y);
    vector<Point>pp(p);
    Point p0 = *min_element(p.begin(), p.end(), part2comp_min);
    P=p0, px=P.getX();
    p.erase(find(p.begin(), p.end(), p0));
    sort(p.begin(), p.end(), part2comp_sort);
    stack<Point> hull;
    hull.push(p[0]), hull.push(p[1]);
    for (int i=2; i<(int)p.size(); i++) {
        Point pt = p[i];
        Point top = hull.top();
        hull.pop();
        Point next = hull.top();
        while (hull.size()>1 && ccw(next, top, pt)<=0) {
            top = hull.top();
            hull.pop();
            next = hull.top();
        }
        if (ccw(next, top, pt) > 0)
            hull.push(top);
        hull.push(pt);
    }
    hull.push(p0);
    vector<Point> out;
    while (!hull.empty()) {
        out.push_back(hull.top());
        hull.pop();
    }
    for (Point& pt : pp) {
        if (find(out.begin(), out.end(), pt) != out.end())
            Render::circle(pt, 3/800.0, 255, 0, 0), Render::circle(pt, 4/800.0, 255, 0, 0);
        else
            Render::circle(pt, 3/800.0, 0, 0, 0);
    }
    ...
    for (int i=0; i<(int)out.size()-1; i++)
        Render::segment(out[i], out[i + 1]);
    Render::segment(out.back(), out.front());
    writePPM("grahamscan.ppm");
}
